      ...
      if(duplicateId.length>0){
        return new Promise(async (resolve, reject)=> {
          courseModel.courseModel.findByIdAndUpdate(duplicateId[0]._id,
            {code:course.courseId, title:course.title, term:course.term, instructor:course.instructor, startTime:course.startTime, endTime: course.endTime, email:course.email},
            function(err, data){
            if (err) {console.error(err);
              reject('Could not find any goal');
            } else {
